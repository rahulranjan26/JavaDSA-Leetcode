...
public class PathToLeafFromRoot {
    ...
    public static void main(String[] args) {
        ...
        int hi = 18;
        int lo = 3;
        findPathToLeaf(root, "", 0, hi, lo);
        System.out.println(path);

    }

    private static void findPathToLeaf(Node node, String s, int sum, int hi, int lo) {
        if (node == null) {
            return;
        }
        ...
        findPathToLeaf(node.left, s + node.val + "->", sum + node.val, hi, lo);
        findPathToLeaf(node.right, s + node.val + "->", sum + node.val, hi, lo);
